...
export const SlideEffect: React.FC<EffectRendererProps> = ({
  ...
  useEffect(() => {
    ...
    timers.current.push(t);
    return () => timers.current.forEach(clearTimeout);
  }, [isActive, onDeleteComplete, pauseAfterTypeMs, allowDelete, prefersReducedMotion]);

  if (!isActive) return null;

  if (prefersReducedMotion) {
    return (
      <span className="text-sm text-[var(--text-tertiary)] select-none">{text}</span>
    );
  }
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="bg-transparent outline-none text-sm text-[var(--text-primary)] placeholder:text-transparent w-full"
